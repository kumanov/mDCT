...
@echo.
@echo. mDCT updates on: https://github.com/
...
call :logitem collecting gpresult /h
...
:WmicProductList
call :logitem Installation package task management - get name, version
call :doCmd  wmic /output:!_DirWork!\GeneralSystemInfo\InstallList.txt product get name,version
...
call :doCmd xcopy /s/e/i/q/y/H "%HwProgramData%\HMIWebLog\*.txt" "!_DirWork!\Station-logs\"
call :doCmd xcopy /s/e/i/q/y/H "%HwProgramData%\HMIWebLog\Archived Logfiles\*.txt" "!_DirWork!\Station-logs\Rollover-logs\" 
...
if exist "%HwProgramData%\Experion PKS\logfiles\logServer.txt" (
	...
	call :doCmd xcopy /s/e/i/q/y/H "%HwProgramData%\Experion PKS\logfiles\log*.txt" "!_DirWork!\SloggerLogs\"
)
:: R4xx
if exist "%HwProgramData%\Experion PKS\Server\data\log.txt" (
	call :logitem get Experion log files
	call :doCmd xcopy /s/e/i/q/y/H "%HwProgramData%\Experion PKS\Server\data\*log*.txt" "!_DirWork!\SloggerLogs\"
)
...
if exist "%HWCONFIGSTUDIOLOGPATH%\Configuration Studio.log" (
	...
	call :doCmd xcopy /s/e/i/q/y/H "%HWCONFIGSTUDIOLOGPATH%\Configuration Studio.log" "!_DirWork!\Configuration Studio\"
)
...
if exist "%HwProgramData%\Experion PKS\Server\data\Report\" (
	...
	call :doCmd xcopy /s/e/i/q/y/H "%HwProgramData%\Experion PKS\Server\data\Report\filrep*.*" "!_DirWork!\ServerDataDirectory\File-Replication\" /s
)
...
if %errorlevel%==0 (
    ...
    call :logCmd !_DirWork!\ServerDataDirectory\shheap.4.output.txt shheap 4 struct
)

:lisscn
where lisscn >NUL 2>&1
if %errorlevel%==0 (
	call :logitem lisscn -all_ref
	call :mkNewDir !_DirWork!\ServerDataDirectory
	call :doCmd lisscn -all_ref -OUT !_DirWork!\ServerDataDirectory\lisscn-all_ref.txt
)
...
if %errorlevel%==0 (
	call :logitem Experion system flags output
	call :mkNewDir !_DirWork!\ServerDataDirectory
    call :doCmd fildmp -DUMP -FILE !_DirWork!\ServerDataDirectory\sysflg.output.txt -FILENUM 8 -RECORDS 1 -FORMAT HEX  1>NUL 2>NUL
)
...
if exist "%HwProgramData%\Experion PKS\Server\data\OPCIntegrator\" (
	...
	call :doCmd xcopy /s/e/i/q/y/H "%HwProgramData%\Experion PKS\Server\data\OPCIntegrator\*.tsv" "!_DirWork!\OPCIntegrator\"
)
...
if exist "%HwProgramData%\Experion PKS\Server\data\" (
	...
	call :mkNewDir !_DirWork!\ServerDataDirectory
	call :doCmd xcopy /s/e/i/q/y/H "%HwProgramData%\Experion PKS\Server\data\*.bad" !_DirWork!\ServerDataDirectory\
)
...
where what >NUL 2>&1
if %errorlevel%==0 (
	call :logitem What - Experion exe/dll information
...
set cabName=mDCT_%_Comp_Time%.cab
...
call :WriteHost yellow black *** Please compress all files in %_DirWork% and upload zip file to GTAC ftp site
call :play_attention_sound
